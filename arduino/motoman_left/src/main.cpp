...
#define STALL_VALUE_HOME 22 //20  // 38 prev:20, For homing, [0..255] if (SG_THRS*2 > SG_RESULT) stall detected
#define STALL_VALUE_NORMAL 0 //  0 For normal operation, but now we dont use stall detect for normal operation
#define RMS_NORMAL 1000 // when motor moving
#define RMS_HOMING 800
...
{ 
  ...
  if (goal_position < 0 ) goal_position = 0;  //clamp goal position
  else if (goal_position > 100 ) goal_position = 100;
...
void driverParamInit()
{
  ...
  driver.rms_current(400);             // mA max 1.5
  ...
  driver.semax(0);
  driver.sedn(0b01);
  driver.pdn_disable(false);   // true: PDN_UART input fun                                                                                                                                                                                                         ction disabled. Set this bit, when using the UART interface
...
void initHoming()
{
  ...
  driver.rms_current(RMS_HOMING); // 150
  driver.SGTHRS(STALL_VALUE_HOME); // 23
  if (driver.stealth())
    nh.loginfo("MODE STEALTH");
  else
    nh.loginfo("MODE SPREADCYCLE");
